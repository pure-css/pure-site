...
exports.setCustomGridFont = setCustomGridFont;
...
function setGridClasses(mediaQueries, options) {
    ...
    mediaQueries.forEach(function (mq) {
        classnames.push(new Handlebars.SafeString(prefix + mq.id + '-1-' + denominator));
        denominator *= 2;
    });

    return new Handlebars.SafeString(classnames.join(' '));
}

function length(arr, options) {
    var options = (options && options.hash) || {},
        add = options.add ? options.add : 0;
    return new Handlebars.SafeString(arr.length + add);
}

function setCustomGridFont (font) {
    var css = 'body, .pure-g [class *= "pure-u"] {\n' +
                '   font-family: ' + font + '\n' +
                '}\n';
