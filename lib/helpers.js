...
exports.sectionHeading = function (heading, options) {
    options = (options && options.hash) || {};
    var tagname    = options.tagname ? options.tagname : 'h2';
    var classnames = options.classnames ? options.classnames : 'content-subhead';
    ...
    var id = options.id ? options.id : heading.toLowerCase()
                .replace(/&[^\s;]+;?/g, '')
                .replace(/[^\s\w\-]+/g, '')
                .replace(/\s+/g, '-');

    var html = '<' + tagname + ' id="' + id + '" class="' + classnames + '">' +
