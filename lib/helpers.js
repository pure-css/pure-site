...
exports.addCdnCSS = function (version, module, options) {
    ...
    if (options.kimono) {
        url = ['kimono', version, module + '-min.css'].join('/')
    }
    else {
        url = [version, 'build', module, module + '-min.css'].join('/');
    }
...
exports.addCustomCSS = function (path, options) {
    var css = this.localCSS || (this.localCSS = []);
    css[options.hash.prepend ? 'unshift' : 'push'](path);
};
...
exports.customCSS = function (options) {
    var urls   = this.localCSS,
        output = '';
